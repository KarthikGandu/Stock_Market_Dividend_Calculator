...
st.title("Welcome to Dividend Calculator")
sidebar = st.sidebar
...
# Define the function to get frequency details
def get_frequency_details(col, df):
    return df.groupby(col)[col].count().sort_values(ascending=False).apply(lambda x: '{:.2f}%'.format(x/len(df)*100)).to_dict()
...
# Create a Streamlit expander to show the graphs
graphs = st.expander("Click here to see a graph.")

# Add a pie plot to the expander
with graphs:
    col1, col2 = st.columns(2)
    with col1:
        # Get the frequency details for the Sector column
        df_info_dict = get_frequency_details("Sector", df)
        
        # Extract the keys and values for the pie plot
        keys = list(df_info_dict.keys())
        vals = [float(df_info_dict[k][:-1]) for k in keys]
        
        # Create the pie plot
        fig1, ax1 = plt.subplots(figsize=(12,8),facecolor='white')
        plt.style.use("dark_background")
        sns.set(style="darkgrid")
        ax1.set_title("Distribution of Stocks in Sectors", size='16', fontweight='bold')
